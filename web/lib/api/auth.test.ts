...
vi.mock('@supabase/supabase-js', () => ({
  createClient: vi.fn(() => ({
    from: vi.fn(() => ({
      ...
      single: vi.fn().mockResolvedValue({ data: { id: 'key-id', user_id: 'user-1' }, error: null }),
...
describe('requireAuthFromHeader', () => {
  ...
  it('routes Bearer token to jwt auth', async () => {
    const result = await requireAuthFromHeader(makeRequest('Bearer fake-jwt-token'))
    expect(result.response).toBeNull()
    expect(result.user?.id).toBe('user-1')
  })

  it('routes ApiKey to api key auth', async () => {
    const result = await requireAuthFromHeader(makeRequest('ApiKey snz_abc123'))
    expect(result.response).toBeNull()
    expect(result.user?.id).toBe('user-1')
  })
