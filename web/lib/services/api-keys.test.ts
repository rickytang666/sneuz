...
function makeClient(overrides: Record<string, unknown> = {}) {
  ...
  return { from: vi.fn(() => chain), _chain: chain } as unknown as Parameters<typeof listKeys>[0] & { _chain: typeof chain }
}

describe('listKeys', () => {
  it('returns keys for the user', async () => {
    const rows = [{ id: '1', name: 'home', created_at: '2026-01-01', last_used_at: null }]
    const client = makeClient()
    client._chain.order = vi.fn().mockResolvedValue({ data: rows, error: null })
  ...
  it('returns empty array when data is null', async () => {
    const client = makeClient()
    client._chain.order = vi.fn().mockResolvedValue({ data: null, error: null })
